...
def edit_post(request:HttpRequest, hash, id):
    ...
    if request.method=='POST':
        ...
        try:
            try:
                requestImgSave(request.FILES['1']) if request.FILES['1'] else False
                editMSG=True
            except:pass
            sleep(0.5)
            try:
                requestImgSave(request.FILES['2']) if request.FILES['2'] else False
                editMSG=True
            except:pass
            sleep(0.5)
            try:
                requestImgSave(request.FILES['3']) if request.FILES['3'] else False
                editMSG=True
            except:pass
            sleep(0.5)
            try:
                requestImgSave(request.FILES['4']) if request.FILES['4'] else False 
                editMSG=True
            except:pass
        except:pass
        sleep(0.5)
