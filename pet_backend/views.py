...
def home(request):
    ...
    if verify0==1 and verify01==0:
        ...
        verify_list=[]
        test_verify = str(verify00(valor0.id))
        verify_list.append(test_verify) if str(verify00(valor0.id))=='True' else False
        if len(verify_list)>0 and verify_list[0] == 'True':
...
def perfil_user(request, key_inst):
    ...
    context={
        'key_inst':valor0.enctype_hash,
        'login_display':'none',
        'username':valor0.nome,
        'user_id':valor0.id,
    }
...
def complemento_info(request):
    ...
    if request.method=='GET':
        ...
        test_verify = str(verify00(valor0.id))
        verify_list.append(test_verify) if str(verify00(valor0.id))=='True' else False
        context['notification']=True if len(verify_list)>0 and verify_list[0] == 'True' else False 
...
def post_detail(request, id, hash, name):
    global valor0
    ...
    if verify0==0 and request.method=='GET':
        try:
            ...
            for test in PubInfo.objects.all():
                try:
                    hash=user_info.objects.get(pk=test.creator)
                    context['key_user']=hash.enctype_hash
...
def delete_post(request, id, hash):
    ...
    if request.method == "POST":
        list_del=[]
        for db_delete in PubInfo.objects.all():
            if db_delete.creator == valor0.id:
                id_delete = db_delete.id
            try:
                if db_delete.creator==id_delete:
                    list_del.append(db_delete.id)
            except:
                pass
            if db_delete.creator == id:
                list_del.append(db_delete.id)
        for n in range(0, len(list_del)):
            del_0 = PubInfo.objects.get(pk=int(list_del[n]))
    ...
    if request.method=='POST':
        ...
        try:
            ...
            mensage_sent=RequisicaoAdotar(
                remetente=valor0.id,
                destinatario=user_info.objects.get(pk=int(destinatario0)).id,
                pub_selec=id_pub,
                msg=request.POST.get('message_input'),
                )
            mensage_sent.save()
            print('INIT SAVE0')
            return redirect(home) 
